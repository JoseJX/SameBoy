...
uint8_t reverseValue(uint8_t v) {
	return revTable[v];
}
...
static uint8_t read_high_memory(GB_gameboy_t *gb, uint16_t addr)
{
    ...
    if (addr < 0xFF80) {
        ...
        switch (addr & 0xFF) {
            ...
            case GB_IO_STAT:
                return reverseValue(gb->io_registers[GB_IO_STAT] | 0x80);
            case GB_IO_OPRI:
                if (!GB_is_cgb(gb)) {
                    return 0xFF;
                }
    ...
    if (addr < 0xFF80) {
	    ...
	    case GB_IO_LCDC_PKT:
            ...
            */
	    	...
	    	GB_log(gb, "Wrote %02x to %04x (HW Register) ROM: 0x%02X PC: 0x%04X\n", value, addr, (gb->pc >= 0x4000) ? gb->mbc_rom_bank : 0, gb->pc);
                return;
        }
    }
